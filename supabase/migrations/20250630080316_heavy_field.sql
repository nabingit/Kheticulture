/*
  ...
  1. New Tables
    - `custom_domains` - Stores domain records for farmers
      - `id` (uuid, primary key)
      - `user_id` (uuid, references profiles)
      - `domain` (text, unique)
      - `entri_domain_id` (text, nullable)
      - `status` (text, default 'pending')
      - `verification_code` (text, nullable)
      - `verified_at` (timestamp, nullable)
      - `created_at` (timestamp)
      - `updated_at` (timestamp)
    - `domain_settings` - Stores appearance settings for domains
      - `domain_id` (uuid, primary key, references custom_domains)
  ...
  2. Views
    - `domain_details` - Combined view of domains with settings

  3. Functions
    - `update_domain_timestamp()` - Update timestamps for domain tables
    - `verify_domain()` - Verify domain ownership
    - `activate_domain()` - Activate a verified domain
    - `generate_domain_verification_code()` - Generate verification code
    - `check_domain_limit()` - Check if user is under domain limit (3)
    - `lookup_domain()` - Find domain details by name
  
  4. Security
    - Enable RLS on both tables
    - Add policies for domain management
    - Grant execute permission on functions
*/

-- Create custom domains table only if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'custom_domains') THEN
...
-- Create domain settings table only if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'domain_settings') THEN
...
DO $$
BEGIN
  -- Drop trigger for custom_domains if it exists
  IF EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_custom_domains_timestamp') THEN
    DROP TRIGGER update_custom_domains_timestamp ON custom_domains;
  END IF;
  
  -- Drop trigger for domain_settings if it exists
  IF EXISTS (SELECT 1 FROM information_schema.triggers WHERE trigger_name = 'update_domain_settings_timestamp') THEN
    DROP TRIGGER update_domain_settings_timestamp ON domain_settings;
  END IF;
END $$;
...
DO $$
BEGIN
  -- Drop policies for custom_domains if they exist
  DROP POLICY IF EXISTS "Users can create their own domains" ON custom_domains;
  DROP POLICY IF EXISTS "Users can read their own domains" ON custom_domains;
  ...
  DROP POLICY IF EXISTS "Users can only create domains up to their limit" ON custom_domains;
  DROP POLICY IF EXISTS "Public can read active domain info" ON custom_domains;
  
  -- Drop policies for domain_settings if they exist
  DROP POLICY IF EXISTS "Users can manage settings for their own domains" ON domain_settings;
END $$;
...
-- Create a policy to allow reading active domains
...
-- Create or replace the view to get domains with their settings
DROP VIEW IF EXISTS domain_details;
CREATE VIEW domain_details AS
SELECT
  d.id,
  d.user_id,
  d.domain,
